...
│   │   ├── blog/           # Content/blog
│   │   ├── sitemap/        # HTML sitemap utility page (/sitemap)
...
generate-rss.mjs            # Post-build script → out/rss.xml
```
...
- Internationalization and translation files managed via `next-intl`
- Locale detection and routing via root-level client-side redirect in `app/page.tsx` (static export compliant)
- Mandatory locale prefixes (`/en`, `/es`) enforced for all sub-paths to resolve static 404 errors
...
2. `postbuild` hook runs `generate-rss.mjs` → writes `out/rss.xml` (4 blog items)
3. Output directory (`out/`) is deployed to Cloudflare Pages
4. No server-side rendering or API routes in production

**Active SEO endpoints in `out/`:**
- `/sitemap.xml` — XML sitemap (Next.js native, localized paths)
- `/robots.txt` — crawl rules (Next.js native)
- `/rss.xml` — RSS feed (generated by `postbuild` hook)
- `/sitemap` — HTML sitemap page (static, includes all locale-prefixed variants)
...
- SEO enhancements (including global injections of JSON-LD Schema structures for `Organization`, `Brand`, and `Service` identifying the Landing site strictly as the public-facing entity of Cyboring Technologies LLC; RSS feed at `/rss.xml` generated post-build; HTML sitemap at `/sitemap`)
...
**Update (2026-03-21)**: Resolved critical sitemap 404 errors in production by localizing all sitemap and canonical URLs. Ensured `sitemap.ts` and `generateMetadata` include the mandatory locale prefix (`/es/` or `/en/`) to match the Static Export directory structure and confirmed internal schema URL consistency.
